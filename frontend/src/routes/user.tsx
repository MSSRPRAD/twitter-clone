...
export default function User() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log("data");
      console.log(data);
      setStuff(data.tweets, data.quoted_tweets, data.profile_pics);
...
// Argument of type '(prev: Tweet[]) => { tweets: Tweet[]; }' is not assignable to parameter of type '(prev: Tweet[]) => Tweet[]'. 
  ...
  return (
    <div class="bg-gray-800 w-full">
      ...
      <ul class="list-none">
        <TweetList quote_tweets={quote_tweets()} tweets={tweets() as Tweet[]} profileUrl = {profileUrl()} profile_pics = { profilePics() }/>
      </ul>
    </div>
  );
}
